...
export function GradientDescentAnimatedVisualisation({}) {
    ...
    const tolerance = 0.000000000000000001;

    useEffect(() => {
        if (data.length > 0) {
            const derivative = calculateFirstDerivative(data, slope);
            const newSlope = slope - learningRate * derivative;

            if (Math.abs(newSlope - slope) > tolerance) {
                setTimeout(() => {
                    setSlope(newSlope); // update the slope
                }, 50); // wait 100ms before next update for animation effect
            } else {
                setTimeout(() => {
                    setSlope((Math.random() * 2 * yScale.domain()[1]) / xScale.domain()[1]); // reset slope to new random value
                }, 1000); // wait a bit longer before resetting
            }
        }
    }, [slope, data]);
